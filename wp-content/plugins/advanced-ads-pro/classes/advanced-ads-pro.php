...
class Advanced_Ads_Pro {
	...
	public function wp_enqueue_scripts() {
        ...
        // Include in footer to prevent conflict with cache busting when Autoptimize and NextGen Gallery are used at the same time.
        $in_footer = defined( 'AUTOPTIMIZE_PLUGIN_DIR' );
		
		wp_enqueue_script( 'advanced-ds-pro/front', AAP_BASE_URL . 'assets/advanced-ads-pro.js', array( 'jquery', ADVADS_SLUG . '-advanced-js' ), AAP_VERSION, $in_footer );
	}
	...
	public function can_display_in_header_by_post_type( $can_display ) {
		...
		if ( in_array( $post_type, $options['general']['disable-by-post-types'], true ) ) {
			return false;
		}
	...
	public function placement_content_offsets( $offsets, $options, $placement_opts ) {
		...
		if ( isset( $placement_opts['placement']['type'] ) ) {
			if ( 'post_content_random' === $placement_opts['placement']['type'] ) {
				$rand = mt_rand( 0, absint( $options['paragraph_count'] - 1 ) );
				return array( $rand );
			}
