...
class Advanced_Ads_Pro_Module_Cache_Busting {
    ...
    public function enqueue_scripts() {
        ...
        if ( defined( 'WP_DEBUG' ) && WP_DEBUG ) {
            wp_register_script( 'krux/postscribe', $uriRelPath . 'inc/postscribe.js', array(), '2.0.8', $in_footer );
            wp_register_script( 'advanced-ads-pro/cache_busting', $uriRelPath . 'inc/base.js', array( 'jquery', 'krux/postscribe' ), AAP_VERSION, $in_footer );
        } else {
            // minified
        ...
        $info['moveintohidden'] = defined( 'ADVANCED_ADS_PRO_CUSTOM_POSITION_MOVE_INTO_HIDDEN' );
	...
	private function get_placement_info( $id ) {
		// The information which passive cache-busting (`inc/base.js`) can read.
		// When a new placement option is added and passive cache-busting needs to access it, it should be added to the array.
		$allowed_keys = array( 'id', 'lazy_load', 'test_id', 'layer_placement', 'close', 'inject_by', 'placement_position' );
    ...
    public function ad_needs_backend_request( Advanced_Ads_Ad $ad ) {
        ...
        } elseif ( $is_visitor_conditions || $is_lazy_load || $hidden_without_consent || $specific_days || $cp_placement ) {
	 ...
	 * @return string
	 */
    public function get_ajax_query( $query ) {
        // Request placement.
    ...
    private function create_wrapper( $element_id ) {
        ...
        return '<div class="' . $element_id . '" id="' . $element_id . '"></div>';
    }
