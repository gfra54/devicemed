...
class DM_Wordpress_Suppliers_Posts_Admin_List extends DM_Wordpress_Admin_Submenu_Page
{
	protected $parent_slug = 'devicemed-suppliers';
	protected $page_title = 'Articles fournisseurs';
	protected $menu_title = 'Articles';
	protected $capability = 'manage_options';
	protected $menu_slug = 'devicemed-suppliers-posts';
	...
	public function render()
	{
		...
		if (!empty($_GET['action']) AND !empty($_GET['supplier_post']))
		{
			if ($_GET['action'] == 'publish')
			{
				$suppliers_posts->admin_list_bulk_publish($_GET['supplier_post']);
			}
			elseif ($_GET['action'] == 'draft')
			{
				$suppliers_posts->admin_list_bulk_draft($_GET['supplier_post']);
			}
			elseif ($_GET['action'] == 'pending')
			{
				$suppliers_posts->admin_list_bulk_pending($_GET['supplier_post']);
			}
		}
		...
		$results = $suppliers_posts->admin_list($current_page, $results_per_page, $filters);
		$count_all = $suppliers_posts->admin_list_count_all();
		$count_published = $suppliers_posts->admin_list_count_published();
		$count_drafts = $suppliers_posts->admin_list_count_drafts();
		$count_pendings = $suppliers_posts->admin_list_count_pendings();
		...
		DM_Wordpress_Template::load('suppliers_posts_admin_list', array(
			'page' => $this,
