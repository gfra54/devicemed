...
class DM_Wordpress_Suppliers_Download_Model extends DM_Wordpress_Model
{
	protected $table = 'dm_suppliers_download';
	...
	public function admin_list($page = 1, $limit = 10, $filters = array())
	{
		...
		{
			$where[] = strtr(
				"(:table.supplier_download_title LIKE '%:search%' OR :table_suppliers.supplier_name LIKE '%:search%' OR :table_suppliers_users.supplier_user_nicename LIKE '%:search%')",
				array(
		...
		$results = $wpdb->get_results(strtr(
            ...
            :table.supplier_download_title,
            :table.supplier_download_created,
            :table.supplier_download_modified,
            :table.supplier_download_status,
	...
	public function admin_list_bulk_publish($ids = array())
	{
		...
		return $wpdb->query(
			'UPDATE '.$this->table().
			' SET supplier_download_status = 1'.
	...
	public function admin_list_bulk_draft($ids = array())
	{
		...
		return $wpdb->query(
			'UPDATE '.$this->table().
			' SET supplier_download_status = 0'.
			' WHERE ID IN ('.implode(', ', $where).')'
		);
	}
	public function admin_list_bulk_pending($ids = array())
	{
		...
		return $wpdb->query(
			'UPDATE '.$this->table().
			' SET supplier_download_status = 2'.
			' WHERE ID IN ('.implode(', ', $where).')'
		);
	}
	...
	public function admin_edit_get_download($id)
	{
		global $wpdb;
	...
	public function get_last_downloads_by_supplier_id($supplier_id, $limit = 3)
	{
		global $wpdb;
