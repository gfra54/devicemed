...
<h2>
	...
	<a href="<?php echo esc_attr(DM_Wordpress_Suppliers_Admin_Edit::instance()->url()); ?>" class="add-new-h2">Ajouter</a>
...
<ul class="subsubsub">
	...
	<li class="active"><a href="<?php echo esc_attr($page->url(array('supplier_status' => 1))); ?>"<?php echo (isset($_GET['supplier_status']) AND $_GET['supplier_status'] == 1) ? ' class="current"' : ''; ?>>Actifs <span class="count">(<?php echo $count['active']; ?>)</span></a> |</li>
	<li class="inactive"><a href="<?php echo esc_attr($page->url(array('supplier_status' => 0))); ?>"<?php echo (isset($_GET['supplier_status']) AND $_GET['supplier_status'] == 0) ? ' class="current"' : ''; ?>>Désactivés <span class="count">(<?php echo $count['inactive']; ?>)</span></a></li>
</ul>
...
<p class="search-box">
	<label class="screen-reader-text" for="user-search-input">Chercher dans les fournisseurs :</label>
	<input type="search" id="user-search-input" name="search" value="<?php echo esc_attr($_GET['search']); ?>" />
	<input type="submit" name="" id="search-submit" class="button" value="Chercher dans les fournisseurs"  />
</p><!-- .search-box -->
...
<div class="alignleft actions">
	<select name="user_status">
		...
		<option value="2"<?php echo (isset($_GET['supplier_status']) AND $_GET['supplier_status'] == '2') ? ' selected="selected"' : ''; ?>>En attente</option>
	</select>
...
else
{
	foreach ($results as $row => $result)
	{
		...
		// On récupére le nombre de clic
		$supplierId = $result->ID;
		$sqlNbClicFournisseur = "SELECT * FROM wordpress_dm_supplier_premium_clic WHERE supplier_premium_id=$supplierId";
		$resultNbClicFournisseur = mysql_query($sqlNbClicFournisseur);
		$nbClicFournisseur = mysql_num_rows($resultNbClicFournisseur);
		...
		foreach ($columns as $field => $column)
		{
			...
			{
				case 'supplier_name':
					if($nbClicFournisseur == 0 || $nbClicFournisseur == 1) {
						echo '<strong><a class="row-title" href="'.esc_attr(DM_Wordpress_Suppliers_Admin_Edit::instance()->url(array('supplier_id' => $result->ID))).'" title="Modifier cet élément">'.esc_html($result->supplier_name).'</a> - '. $nbClicFournisseur .' clic</strong>';
					}else {
						echo '<strong><a class="row-title" href="'.esc_attr(DM_Wordpress_Suppliers_Admin_Edit::instance()->url(array('supplier_id' => $result->ID))).'" title="Modifier cet élément">'.esc_html($result->supplier_name).'</a> - '. $nbClicFournisseur .' clics</strong>';
					}
					echo '<div class="row-actions">';
					echo '<span class="edit"><a href="'.esc_attr(DM_Wordpress_Suppliers_Admin_Edit::instance()->url(array('supplier_id' => $result->ID))).'" title="Modifier cet élément">Modifier</a></span>';
					...
					echo '</div>';
				break;
				case 'supplier_category_title':
					echo esc_html($result->supplier_category_title);
				break;
				case 'supplier_status':
					switch ($result->supplier_status)
					{
						case 0:
							echo 'Fournisseur désactivé';
						break;
						case 1:
							echo 'Fournisseur actif';
						break;
						case 2:
							echo 'En attente';
						break;
				...
				break;
				case 'supplier_date':
					echo '<div>Créé le '.date('d/m/Y', strtotime($result->supplier_created)).'</div>';
					echo '<div>Modifié le '.date('d/m/Y', strtotime($result->supplier_modified)).'</div>';
				break;
			}
			echo '</td>';
		}
