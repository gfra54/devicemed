...
<h2>
	...
	<a href="<?php echo esc_attr(DM_Wordpress_Suppliers_Posts_Admin_Edit::instance()->url()); ?>" class="add-new-h2">Ajouter</a>
...
else
{
	foreach ($results as $row => $result)
	{
		...
		echo '<input id="cb-select-'.$result->ID.'" type="checkbox" name="supplier_post[]" value="'.$result->ID.'" />';
		echo '</th>';
		...
		foreach ($columns as $field => $column)
		{
			...
			switch($field)
			{
				case 'supplier_post_title':
					echo '<strong><a class="row-title" href="'.esc_attr(DM_Wordpress_Suppliers_Posts_Admin_Edit::instance()->url(array('supplier_post_id' => $result->ID))).'" title="Modifier cet élément">'.esc_html($result->supplier_post_title).'</a></strong>';
					echo '<div class="row-actions">';
					echo '<span class="edit"><a href="'.esc_attr(DM_Wordpress_Suppliers_Posts_Admin_Edit::instance()->url(array('supplier_post_id' => $result->ID))).'" title="Modifier cet élément">Modifier</a></span>';
					echo '</div>';
				break;
				case 'supplier_name':
				...
				case 'supplier_post_status':
					switch ($result->supplier_post_status)
					{
						case 0:
							echo 'Brouillon';
						break;
