...
<h2>
	...
	<a href="<?php echo esc_attr(DM_Wordpress_Suppliers_Categories_Admin_Edit::instance()->url()); ?>" class="add-new-h2">Ajouter</a>
...
<ul class="subsubsub">
	<li class="all"><a href="<?php echo esc_attr($page->url()); ?>"<?php echo (!isset($_GET['supplier_user_status']) OR $_GET['supplier_user_status'] == -1) ? ' class="current"' : ''; ?>>Toutes <span class="count">(<?php echo $count['all']; ?>)</span></a></li>
</ul>
...
<p class="search-box">
	<label class="screen-reader-text" for="user-search-input">Chercher dans les catégories :</label>
	<input type="search" id="user-search-input" name="search" value="<?php echo esc_attr($_GET['search']); ?>" />
	<input type="submit" name="" id="search-submit" class="button" value="Chercher dans les catégories"  />
</p><!-- .search-box -->
...
else
{
	foreach ($results as $row => $result)
	{
		...
		echo '<input id="cb-select-'.$result->ID.'" type="checkbox" name="supplier_category[]" value="'.$result->ID.'" />';
		echo '</th>';
		...
		foreach ($columns as $field => $column)
		{
			...
			switch($field)
			{
				case 'supplier_category_title':
					echo '<strong><a class="row-title" href="'.esc_attr(DM_Wordpress_Suppliers_Categories_Admin_Edit::instance()->url(array('supplier_category_id' => $result->ID))).'" title="Modifier cet élément">'.esc_html($result->supplier_category_title).'</a></strong>';
					echo '<div class="row-actions">';
					echo '<span class="edit"><a href="'.esc_attr(DM_Wordpress_Suppliers_Categories_Admin_Edit::instance()->url(array('supplier_category_id' => $result->ID))).'" title="Modifier cet élément">Modifier</a></span>';
					echo '</div>';
				break;
			}
			echo '</td>';
