...
<h2>
	...
	<a href="<?php echo esc_attr(DM_Wordpress_Newsletter_Admin_Edit::instance()->url()); ?>" class="add-new-h2">Ajouter</a>
...
<p class="search-box">
	<label class="screen-reader-text" for="user-search-input">Chercher dans les inscrits :</label>
	<input type="search" id="user-search-input" name="search" value="<?php echo esc_attr($_GET['search']); ?>" />
	<input type="submit" name="" id="search-submit" class="button" value="Chercher dans les inscrits"  />
</p><!-- .search-box -->
...
else
{
	foreach ($results as $row => $result)
	{
		...
		echo '<input id="cb-select-'.$result->ID.'" type="checkbox" name="inscrits[]" value="'.$result->ID.'" />';
		echo '</th>';
		...
		foreach ($columns as $field => $column)
		{
			...
			switch($field)
			{
				case 'mail_newsletter':
					echo '<strong><a class="row-title" href="'.esc_attr(DM_Wordpress_Newsletter_Admin_Edit::instance()->url(array('inscrits_id' => $result->ID))).'" title="Modifier cet élément">'.esc_html($result->mail_newsletter).'</a></strong>';
					echo '<div class="row-actions">';
					echo '<span class="edit"><a href="'.esc_attr(DM_Wordpress_Newsletter_Admin_Edit::instance()->url(array('inscrits_id' => $result->ID))).'" title="Modifier cet élément">Modifier</a></span>';
					...
					echo '</div>';
				break;
				case 'offre_devicemed':
					if($result->offre_devicemed == 1) {
						$offreDevicemed = "Oui";
					}elseif($result->offre_devicemed == 2) {
						$offreDevicemed = "Non";
					}
					echo '<div>'. $offreDevicemed .'</div>';
				break;
				case 'offre_partenaires':
					if($result->offre_partenaires == 1) {
						$offrePartenaires = "Oui";
					}elseif($result->offre_partenaires == 2) {
						$offrePartenaires = "Non";
					}
					echo '<div>'. $offrePartenaires .'</div>';
				break;
				case 'nom':
					echo '<div>'. $result->nom .'</div>';
				break;
				case 'prenom':
					echo '<div>'. $result->prenom .'</div>';
				break;
				case 'ville':
					echo '<div>'. $result->ville .'</div>';
				break;
				case 'cp':
					echo '<div>'. $result->cp .'</div>';
				break;
				case 'actif':
					if($result->actif == 1) {
						$actif = 'Oui';
