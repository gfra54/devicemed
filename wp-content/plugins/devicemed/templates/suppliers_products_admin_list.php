...
$bulkactions = array(
	'-1' => 'Actions groupées',
	'publish' => 'Publier les produits',
	'draft' => 'Brouillons',
	'pending' => 'En attente de relecture'
);

$sendback_url = template_build_sendback_url(
	array('search', 'orderby', 'order', 'supplier_product_status', 'paged'),
	$filters
);
...
<h2>
	...
	<a href="<?php echo esc_attr(DM_Wordpress_Suppliers_Products_Admin_Edit::instance()->url()); ?>" class="add-new-h2">Ajouter</a>
...
<ul class="subsubsub">
	<li class="all"><a href="<?php echo esc_attr($page->url()); ?>"<?php echo (!isset($_GET['supplier_product_status']) OR $_GET['supplier_product_status'] == -1) ? ' class="current"' : ''; ?>>Tous <span class="count">(<?php echo $count['all']; ?>)</span></a> |</li>
	<li class="drafts"><a href="<?php echo esc_attr($page->url(array('supplier_product_status' => 0))); ?>"<?php echo (isset($_GET['supplier_product_status']) AND $_GET['supplier_product_status'] == 0) ? ' class="current"' : ''; ?>>Brouillons <span class="count">(<?php echo $count['drafts']; ?>)</span></a> |</li>
	<li class="pendings"><a href="<?php echo esc_attr($page->url(array('supplier_product_status' => 1))); ?>"<?php echo (isset($_GET['supplier_product_status']) AND $_GET['supplier_product_status'] == 1) ? ' class="current"' : ''; ?>>En attente de relecture <span class="count">(<?php echo $count['pendings']; ?>)</span></a> |</li>
	<li class="published"><a href="<?php echo esc_attr($page->url(array('supplier_product_status' => 2))); ?>"<?php echo (isset($_GET['supplier_product_status']) AND $_GET['supplier_product_status'] == 2) ? ' class="current"' : ''; ?>>Publiés <span class="count">(<?php echo $count['published']; ?>)</span></a></li>
</ul>
...
<p class="search-box">
	<label class="screen-reader-text" for="user-search-input">Chercher dans les produits :</label>
	<input type="search" id="user-search-input" name="search" value="<?php echo esc_attr($_GET['search']); ?>" />
	<input type="submit" name="" id="search-submit" class="button" value="Chercher dans les produits"  />
</p><!-- .search-box -->
...
<div class="alignleft actions">
	<select name="user_status">
		<option value="-1"<?php echo (isset($_GET['supplier_product_status']) AND $_GET['supplier_product_status'] == '-1') ? ' selected="selected"' : ''; ?>>Tous les statuts</option>
		<option value="0"<?php echo (isset($_GET['supplier_product_status']) AND $_GET['supplier_product_status'] == '0') ? ' selected="selected"' : ''; ?>>Brouillons</option>
		<option value="1"<?php echo (isset($_GET['supplier_product_status']) AND $_GET['supplier_product_status'] == '1') ? ' selected="selected"' : ''; ?>>En attente de relecture</option>
		<option value="2"<?php echo (isset($_GET['supplier_product_status']) AND $_GET['supplier_product_status'] == '2') ? ' selected="selected"' : ''; ?>>Publiés</option>
	</select>
...
else
{
	foreach ($results as $row => $result)
	{
		...
		echo '<input id="cb-select-'.$result->ID.'" type="checkbox" name="supplier_product[]" value="'.$result->ID.'" />';
		echo '</th>';
		...
		foreach ($columns as $field => $column)
		{
			...
			switch($field)
			{
				case 'supplier_product_title':
					echo '<strong><a class="row-title" href="'.esc_attr(DM_Wordpress_Suppliers_Products_Admin_Edit::instance()->url(array('supplier_product_id' => $result->ID))).'" title="Modifier cet élément">'.esc_html($result->supplier_product_title).'</a></strong>';
					echo '<div class="row-actions">';
					echo '<span class="edit"><a href="'.esc_attr(DM_Wordpress_Suppliers_Products_Admin_Edit::instance()->url(array('supplier_product_title' => $result->ID))).'" title="Modifier cet élément">Modifier</a></span>';
					echo '</div>';
				break;
				case 'supplier_name':
					echo esc_html($result->supplier_name);
				break;
				case 'supplier_product_author':
					echo esc_html($result->supplier_user_nicename);
				break;
				case 'supplier_product_status':
					switch ($result->supplier_product_status)
					{
						case 0:
							echo 'Brouillon';
						break;
				...
				break;
				case 'supplier_product_date':
					echo '<div>Créé le '.date('d/m/Y', strtotime($result->supplier_product_created)).'</div>';
					echo '<div>Modifié le '.date('d/m/Y', strtotime($result->supplier_product_modified)).'</div>';
				break;
			}
			echo '</td>';
		}
