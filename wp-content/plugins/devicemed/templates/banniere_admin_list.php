...
$bulkactions = array(
	...
	'delete' => 'Supprimer les bannières'
);
...
<h2>
	...
	<a href="<?php echo esc_attr(DM_Wordpress_Banniere_Admin_Edit::instance()->url()); ?>" class="add-new-h2">Ajouter</a>
...
<p class="search-box">
	<label class="screen-reader-text" for="user-search-input">Chercher dans les catalogues :</label>
	<input type="search" id="user-search-input" name="search" value="<?php echo esc_attr($_GET['search']); ?>" />
	<input type="submit" name="" id="search-submit" class="button" value="Chercher dans les catalogues"  />
</p><!-- .search-box -->
...
else
{
	foreach ($results as $row => $result)
	{
		...
		echo '<input id="cb-select-'.$result->ID.'" type="checkbox" name="banniere_id[]" value="'.$result->ID.'" />';
		echo '</th>';
		...
		foreach ($columns as $field => $column)
		{
			...
			{
				case 'nom_banniere':
					echo '<strong><a class="row-title" href="'.esc_attr(DM_Wordpress_Banniere_Admin_Edit::instance()->url(array('banniere_id' => $result->ID))).'" title="Modifier cet élément">'.esc_html($result->nom_banniere).'</a></strong>';
					echo '<div class="row-actions">';
					echo '<span class="edit"><a href="'.esc_attr(DM_Wordpress_Banniere_Admin_Edit::instance()->url(array('banniere_id' => $result->ID))).'" title="Modifier cet élément">Modifier</a></span>';
				...
				break;
				case 'date_fin':
					echo '<div>Termine le '.date('d/m/Y', strtotime($result->date_fin)).'</div>';
				break;
				case 'frequence':
					if($result->frequence == 3) {
						echo '<div>Haute</div>';
					}elseif($result->frequence == 2) {
						echo '<div>Moyenne</div>';
					}elseif($result->frequence == 1) {
						echo '<div>Faible</div>';
					}
				break;
				case 'affichage':
					if($result->affichage == 1) {
						echo '<div>Verticale</div>';
					}elseif($result->affichage == 2) {
						echo '<div>Horizontale</div>';
					}elseif($result->affichage == 3) {
						echo '<div>Newsletter</div>';
					}
				break;
				case 'nombre_clic':
					$banniere_model = new DM_Wordpress_Banniere_Model();
					$banniere_clic = $banniere_model->get_clic($result->ID);
					$nbClic = $banniere_clic['nbClic'];
