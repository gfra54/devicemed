...
$sendback_url = template_build_sendback_url(
	array('search', 'orderby', 'order', 'user_status', 'paged'),
	$filters
);
...
<h2>
	...
	<a href="<?php echo esc_attr(DM_Wordpress_Members_Admin_Edit::instance()->url()); ?>" class="add-new-h2">Ajouter</a>
...
<ul class="subsubsub">
	<li class="all"><a href="<?php echo esc_attr($page->url()); ?>"<?php echo (!isset($_GET['user_status']) OR $_GET['user_status'] == -1) ? ' class="current"' : ''; ?>>Tous <span class="count">(<?php echo $count['all']; ?>)</span></a> |</li>
	<li class="active"><a href="<?php echo esc_attr($page->url(array('user_status' => 1))); ?>"<?php echo (isset($_GET['user_status']) AND $_GET['user_status'] == 1) ? ' class="current"' : ''; ?>>Actifs <span class="count">(<?php echo $count['active']; ?>)</span></a> |</li>
	<li class="inactive"><a href="<?php echo esc_attr($page->url(array('user_status' => 0))); ?>"<?php echo (isset($_GET['user_status']) AND $_GET['user_status'] == 0) ? ' class="current"' : ''; ?>>Désactivés <span class="count">(<?php echo $count['inactive']; ?>)</span></a></li>
</ul>
...
<p class="search-box">
	<label class="screen-reader-text" for="user-search-input">Chercher dans les membres :</label>
	<input type="search" id="user-search-input" name="search" value="<?php echo esc_attr($_GET['search']); ?>" />
	<input type="submit" name="" id="search-submit" class="button" value="Chercher dans les membres"  />
</p><!-- .search-box -->
...
<div class="alignleft actions">
	<select name="user_status">
		<option value="-1"<?php echo (isset($_GET['user_status']) AND $_GET['user_status'] == '-1') ? ' selected="selected"' : ''; ?>>Tous les statuts</option>
		<option value="1"<?php echo (isset($_GET['user_status']) AND $_GET['user_status'] == '1') ? ' selected="selected"' : ''; ?>>Comptes actifs</option>
		<option value="0"<?php echo (isset($_GET['user_status']) AND $_GET['user_status'] == '0') ? ' selected="selected"' : ''; ?>>Comptes désactivés</option>
	</select>
...
else
{
	foreach ($results as $row => $result)
	{
		...
		echo '<label class="screen-reader-text" for="cb-select-'.$result->ID.'">Sélectionner Nouveaux grades de polypropylène</label>';
		echo '<input id="cb-select-'.$result->ID.'" type="checkbox" name="member[]" value="'.$result->ID.'" />';
		echo '</th>';
		...
		foreach ($columns as $field => $column)
		{
			...
			{
				case 'user_login':
					echo '<strong><a class="row-title" href="'.esc_attr(DM_Wordpress_Members_Admin_Edit::instance()->url(array('user_id' => $result->ID))).'" title="Modifier cet élément">'.esc_html($result->user_login).'</a></strong>';
					echo '<div class="row-actions">';
					echo '<span class="edit"><a href="'.esc_attr(DM_Wordpress_Members_Admin_Edit::instance()->url(array('user_id' => $result->ID))).'" title="Modifier cet élément">Modifier</a></span>';
				...
				break;
				case 'user_lastname':
					echo esc_html($result->user_lastname);
				break;
				case 'user_firstname':
					echo esc_html($result->user_firstname);
				break;
				case 'user_email':
					echo '<a href="mailto:'.esc_attr($result->user_email).'" title="Envoyer un e-mail">'.esc_html($result->user_email).'</a>';
				break;
				case 'user_status':
				...
				break;
				case 'user_date':
					echo '<div>Créé le '.date('d/m/Y', strtotime($result->user_created)).'</div>';
					echo '<div>Modifié le '.date('d/m/Y', strtotime($result->user_modified)).'</div>';
				break;
			}
			echo '</td>';
		}
