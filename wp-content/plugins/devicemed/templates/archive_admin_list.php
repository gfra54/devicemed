...
$bulkactions = array(
	...
	'delete' => 'Supprimer les catalogues'
);
...
<h2>
	...
	<a href="<?php echo esc_attr(DM_Wordpress_Archive_Admin_Edit::instance()->url()); ?>" class="add-new-h2">Ajouter</a>
...
else
{
	foreach ($results as $row => $result)
	{
		...
		echo '<input id="cb-select-'.$result->ID.'" type="checkbox" name="archive[]" value="'.$result->ID.'" />';
		echo '</th>';
		...
		foreach ($columns as $field => $column)
		{
			...
			{
				case 'titre_archive':
					echo '<strong><a class="row-title" href="'.esc_attr(DM_Wordpress_Archive_Admin_Edit::instance()->url(array('archive_id' => $result->ID))).'" title="Modifier cet élément">'.esc_html($result->titre_archive).'</a></strong>';
					echo '<div class="row-actions">';
					echo '<span class="edit"><a href="'.esc_attr(DM_Wordpress_Archive_Admin_Edit::instance()->url(array('archive_id' => $result->ID))).'" title="Modifier cet élément">Modifier</a></span>';
				...
				case 'date_publication':
					echo '<div>Créé le '.date('d/m/Y', strtotime($result->date_publication)).'</div>';
