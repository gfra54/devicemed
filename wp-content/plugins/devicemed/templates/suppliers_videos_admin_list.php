...
<h2>
	...
	<a href="<?php echo esc_attr(DM_Wordpress_Suppliers_Videos_Admin_Edit::instance()->url()); ?>" class="add-new-h2">Ajouter</a>
...
else
{
	foreach ($results as $row => $result)
	{
		...
		echo '<input id="cb-select-'.$result->ID.'" type="checkbox" name="supplier_video[]" value="'.$result->ID.'" />';
		echo '</th>';
		...
		foreach ($columns as $field => $column)
		{
			...
			switch($field)
			{
				case 'supplier_video_title':
					echo '<strong><a class="row-title" href="'.esc_attr(DM_Wordpress_Suppliers_Videos_Admin_Edit::instance()->url(array('supplier_video_id' => $result->ID))).'" title="Modifier cet élément">'.esc_html($result->supplier_video_title).'</a></strong>';
					echo '<div class="row-actions">';
					echo '<span class="edit"><a href="'.esc_attr(DM_Wordpress_Suppliers_Videos_Admin_Edit::instance()->url(array('supplier_video_id' => $result->ID))).'" title="Modifier cet élément">Modifier</a></span>';
					echo '</div>';
				break;
				case 'supplier_name':
					echo esc_html($result->supplier_name);
				break;
				case 'supplier_video_author':
					echo esc_html($result->supplier_user_nicename);
				break;
				case 'supplier_video_status':
					switch ($result->supplier_video_status)
					{
						case 0:
							echo 'Brouillon';
						break;
				...
				break;
				case 'supplier_video_date':
					echo '<div>Créé le '.date('d/m/Y', strtotime($result->supplier_video_created)).'</div>';
					echo '<div>Modifié le '.date('d/m/Y', strtotime($result->supplier_video_modified)).'</div>';
				break;
			}
			echo '</td>';
		}
