(function($) {
    $(document).ready(function() {
        if ($('[data-name="contenu_mail"]').length) {
            $('<div style="border: 2px solid #3c3c3c;margin: 2em;padding: 2em;border-radius: 1em;font-face:sans-serif" class="acf-preview"></div>').prependTo('[data-name="contenu_mail"] .acf-input');
            $('[data-name="contenu_mail"] textarea').on('change keyup', function(e) {
                console.log($(e.target).val())
                $('[data-name="contenu_mail"] .acf-preview').html(nl2br($(e.target).val().replace()));
            });
            $('[data-name="contenu_mail"] .acf-preview').html(nl2br($('[data-name="contenu_mail"] textarea').val()));
        }
