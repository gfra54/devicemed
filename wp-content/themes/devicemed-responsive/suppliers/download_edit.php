...
<div class="row column-content page-suppliers page-suppliers-products page-suppliers-products-edit">
	<div class="col-md-9 col-sm-8 column-main">
		<section>
			<h2 class="title"><?php echo $data['ID'] ? 'Modifier un téléchargement' : 'Ajouter un téléchargement'; ?></h2>
			<div class="form-fieldset">
				<form method="post" enctype="multipart/form-data">
					<div class="form-row form-row-title">
						<div class="form-label"><label for="supplier_product_title">Nom du fichier</label></div>
						<div class="form-field"><input type="text" id="supplier_download_title" name="supplier_download_title" value="<?php echo esc_attr($data['supplier_download_title']); ?>" placeholder="Nom du fichier" /></div>
						<div class="form-message"><?php if (!empty($errors['supplier_download_title'])): ?><div class="form-error"><?php echo $errors['supplier_download_title']; ?></div><?php endif; ?></div>
					</div>
					<div class="form-row">
						<div class="form-label"><label>Image</label></div>
						<?php if($data['ID']) { ?>
							<img src='http://www.device-med.fr/wp-content/uploads/suppliers/downloads/apercu/<?php echo $data['ID']; ?>/<?php echo $data['supplier_download_apercu']; ?>' />
						<?php } ?>
						<div class="form-field form-field-files">
							<div class="fileupload-wrapper">
								<label for="fileupload">Ajouter un fichier : </label><input id="fileupload" type="file" name="image_download" accept="image/*" multiple>
							</div>
						</div>
					</div>
					<div class="form-row">
						<div class="form-label"><label>PDF</label></div>
						<div class="form-field form-field-files">
							<div class="fileupload-wrapper">
								<label for="fileupload">Catalogue (au format PDF) : </label><input id="fileupload" type="file" name="pdf_download" accept="application/pdf" multiple>
							</div>
						</div>
					</div>
					<div class="form-submit"><input type="submit" value="<?php if ($data['ID']): ?>Modifier le téléchargement<?php else: ?>Ajouter le téléchargement<?php endif; ?>" /></div>
				</form>
			</div>
		</section>
	</div><!-- .column-main -->
