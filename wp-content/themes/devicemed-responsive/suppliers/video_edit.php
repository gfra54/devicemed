...
<?php } ?>
<div class="row column-content page-suppliers page-suppliers-videos page-suppliers-videos-edit">
	<div class="col-md-9 col-sm-8 column-main">
		<section>
			<h2 class="title"><?php echo $data['ID'] ? 'Modifier vidéo' : 'Ajouter vidéo'; ?></h2>
			<div class="form-fieldset">
				<form method="post">
					<div class="form-row form-row-title">
						<div class="form-label"><label for="supplier_video_title">Titre de la vidéo</label></div>
						<div class="form-field"><input type="text" id="supplier_video_title" name="supplier_video_title" value="<?php echo esc_attr($data['supplier_video_title']); ?>" placeholder="Titre de la vidéo" /></div>
						<div class="form-message"><?php if (!empty($errors['supplier_video_title'])): ?><div class="form-error"><?php echo $errors['supplier_video_title']; ?></div><?php endif; ?></div>
					</div>
<?php
if ($data['supplier_video_media']):
	$mediasTemp = DM_Wordpress_Suppliers_Videos::get_medias($data['ID']);
	$media = $mediasTemp[0];
?>
					<div class="form-row">
					<div class="form-label"><label>Vidéo</label></div>
...
<?php endif; ?>
					<div class="form-row">
						<div class="form-label"><label>Lien de la vidéo</label></div>
						<?php 
							if($data['ID']) {
								$data['supplier_video_content'] = $media;
							}
						?>
						<div class="form-field form-field-video-content"><input type="text" name="supplier_video_content" placeholder="Copier ici le code d'intégration, ou le lien de la vidéo" value="<?php echo esc_attr($data['supplier_video_content']); ?>" /></div>
						<div class="form-message"><?php if (!empty($errors['supplier_video_content'])): ?><div class="form-error"><?php echo $errors['supplier_video_content']; ?></div><?php endif; ?></div>
						<div class="note">Hébergeurs pris en charge : Youtube, Dailymotion, Vimeo.</div>
					</div>
					<div class="form-submit"><input type="submit" value="<?php if ($data['ID']): ?>Modifier vidéo<?php else: ?>Ajouter vidéo<?php endif; ?>" /></div>
				</form>
			</div>
		</section>
	</div><!-- .column-main -->
